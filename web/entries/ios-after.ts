...
(function () {
  ...
  function filterAll() {
    // 핵심 로직은 web/core/filter-engine.ts. iOS 는 badge 가 없어 카운트 무시.
  ...
  function sendBlockMessage(pid, nickname) {
    if (window.webkit && window.webkit.messageHandlers && window.webkit.messageHandlers.qlBridge) {
      window.webkit.messageHandlers.qlBridge.postMessage(
        JSON.stringify({
          type: 'BLOCK_USER',
  ...
  // ── UI Injector (차단 버튼) ──
  // 핵심 path A / path B / cleanbot 가드는 web/core/inject-buttons.ts. iOS 책임은 (a) ql-btn
  ...
  const injectButtonsAdapter: InjectButtonsAdapter = {
    ...
    createButton() {
      ...
      btn.textContent = '✕';
      btn.style.cssText =
        'margin-left:6px;cursor:pointer;opacity:0.5;font-size:16px;border:none;background:rgba(200,50,50,0.12);padding:4px 8px;color:#e74c3c;border-radius:4px;transition:opacity 0.15s;line-height:1;min-width:28px;min-height:28px;display:inline-flex;align-items:center;justify-content:center;';
