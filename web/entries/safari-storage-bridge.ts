// @ts-nocheck
// QuietLounge — Safari Web Extension Storage Bridge (ISOLATED world, document_start)
//
// 이 파일은 esbuild 가 IIFE 로 번들해
// safari-extension/QuietLounge/Shared (Extension)/Resources/content-scripts/storage-bridge.js
// 산출물을 만든다. manifest.json 의 content_scripts 첫 번째 항목에서 가장 먼저 실행되어
// `globalThis.__QL_storage` 헬퍼를 노출 — 이후 main.js 등 다른 ISOLATED 스크립트가 이걸 통해
// App Group(group.kr.konempty.quietlounge) UserDefaults 와 공유된 저장소에 접근.
//
// 동작 요약:
//   - background service-worker → SafariWebExtensionHandler → App Group UserDefaults 위임
//   - macOS Safari 는 native 앱과 공유할 데이터가 없어 bridge 비활성화 (sandboxed browser.storage.local 사용)
//   - browser.storage.local 의 get/set/remove 도 함께 오버라이드해 호환성 확보
//   - 첫 install 시 legacy browser.storage.local → App Group 으로 1 회 마이그레이션
...
(function () {
  ...
  // 현대 iPadOS 는 Request Desktop Site 가 기본이라 navigator.userAgent 가 Macintosh 를 포함 —
  // maxTouchPoints 로 iPad 와 진짜 Mac 을 구분.
  ...
  if (isMacOS) {
    return; // __QL_storage 미노출 → popup/content script 가 browser.storage.local 직접 사용
  }
  ...
  // ── 3) 레거시 browser.storage.local 데이터 1 회 마이그레이션 ──
