...
//
// 4 플랫폼 차이는 ProfileStatsAdapter 로 흡수: 브랜드 색 / 본인 프로필 영속화 / 내 통계 영속화 (Chrome·Safari 만).
//
// 핵심 흐름:
//   1. SPA 네비게이션 → `injectProfileStats()` 호출 (entry 측이 popstate / history.pushState 감시)
//   2. personaId 추출 → 캐시 hit 면 `startProfileStatsGuard()` 만, miss 면 `fetchPersonaStatsWithRetry()` 후 동일
//   3. createTime 이 이번달이면 totalPostCount/totalCommentCount 그대로 (월 카운트 = 총 카운트),
//      아니면 `fetchMonthlyCount('posts'|'comments')` 로 페이지네이션 순회
//   4. `startProfileStatsGuard()`: rAF 폴링 3 초 (빠른 리렌더 대응) → MutationObserver 로 전환
//
// 캐시는 module-level — 같은 personaId 재방문 시 fetch 재호출 방지. in-flight 가드로 page-level
