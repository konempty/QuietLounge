...
| 플랫폼               | 폴링 방식       | 백그라운드      | 알림                       |
|-------------------|-------------|------------|--------------------------|
| Chrome 확장         | Alarms API  | O (서비스 워커) | chrome.notifications     |
| iOS 네이티브 앱        | Timer       | X (포그라운드만) | UNUserNotificationCenter |
| Android (Expo)    | setInterval | X (포그라운드만) | expo-notifications       |
| Safari 확장 (popup) | 미구현         | -          | -                        |

> 앱(iOS/Android)에서는 앱 사용 중에만 설정한 주기마다 확인하며, 앱을 닫았다가 다시 열면 그동안의 새 글을 확인하여 알림을 한번에 보낸다.
