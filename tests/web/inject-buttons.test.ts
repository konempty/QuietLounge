// 차단 버튼 inject 단위 테스트.
//
// web/core/inject-buttons.ts 가 path A / path B / cleanbot 가드 / pid 미매핑 strategy /
// bfcache shouldSkipExistingButton 같은 핵심 동작을 정확히 처리하는지 — 4 entry 가 같은 함수를
// 사용하므로 단일 source 검증으로 4 플랫폼 회귀를 막는다.

import { describe, it, expect, beforeEach, vi } from 'vitest';
import { JSDOM } from 'jsdom';
import { injectBlockButtons, findPersonaId } from '../../web/core/inject-buttons';
import type { InjectButtonsAdapter } from '../../web/platform/adapter';
