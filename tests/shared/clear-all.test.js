...
describe('Chrome 팝업 — 전체 삭제 동작', () => {
  ...
  it('차단 0명일 때 — alert 만 띄우고 confirm 호출 안 함', async () => {
    const ctx = await setupPopup();
    dom = ctx.dom;
    ctx.win.document.getElementById('btn-clear-all').click();
    for (let i = 0; i < 5; i++) await new Promise((r) => setTimeout(r, 0));

    expect(ctx.alertSpy).toHaveBeenCalledWith('차단된 유저가 없습니다.');
    expect(ctx.confirmSpy).not.toHaveBeenCalled();
  });

  it('차단 유저 있을 때 confirm true → 모두 초기화', async () => {
    const seed = {
    ...
    // 메시지 안에 총 인원수 + 되돌릴 수 없음 경고 포함 검증
    expect(ctx.confirmSpy.mock.calls[0][0]).toContain('2');
